...
def genera_biglietto_visita(membro_data, conf):
    from fpdf import FPDF
    import os
    
    # 1. Estrazione Dati Personali
    nome = membro_data.get('nome', '')
    cognome = membro_data.get('cognome', '')
    ruolo = membro_data.get('ruolo', '')
    email = membro_data.get('email', '')
    telefono = membro_data.get('telefono', '') if str(membro_data.get('telefono', '')) != 'nan' else ''
    linkedin = membro_data.get('linkedin', '') if str(membro_data.get('linkedin', '')) != 'nan' else ''
    
    # Dati Istituzionali
    nome_ass = conf.get('nome_associazione', 'ASSAFRICA')
    sito = conf.get('sito_web', 'www.assafrica.it')
    indirizzo = conf.get('indirizzo', '')
    email_ass = conf.get('email_contatto', '')

    # 2. Creazione della vCard
    vcard = f"""BEGIN:VCARD
VERSION:3.0
N:{cognome};{nome};;;
FN:{nome} {cognome}
...
END:VCARD"""
    ...
    if not os.path.exists("exports"): os.makedirs("exports")
    qr_path = f"exports/temp_qr_{nome}_{cognome}.png"
    img_qr.save(qr_path)

    # 4. Creazione PDF (Formato Orizzontale Forzato)
    pdf = FPDF(orientation='P', unit='mm', format=(85, 55))
    pdf.set_auto_page_break(auto=False, margin=0) 
    BLUE_ASSAFRICA = (0, 45, 90)

    # ==========================================
    # LATO 1: FRONTE (Bianco)
    # ==========================================
    pdf.add_page()
    ...
    pdf.set_text_color(*BLUE_ASSAFRICA)
    nome_pulito = f"{nome} {cognome}".encode('latin-1', 'ignore').decode('latin-1')
    pdf.cell(48, 6, nome_pulito, 0, 1, 'L')
    
    # Ruolo su più righe
    pdf.set_xy(6, 18)
    pdf.set_font("helvetica", "I", 8)
    pdf.set_text_color(100, 100, 100)
    ruolo_pulito = str(ruolo).encode('latin-1', 'ignore').decode('latin-1')
    pdf.multi_cell(48, 4, ruolo_pulito, 0, 'L')
    
    # Posizione dinamica per i contatti
    y_pos = pdf.get_y() + 4
    ...
    if email:
        pdf.set_xy(6, y_pos)
        pdf.cell(48, 4, f"E: {email}", 0, 1, 'L')
        y_pos += 4
    if telefono:
        pdf.set_xy(6, y_pos)
        pdf.cell(48, 4, f"T: {telefono}", 0, 1, 'L')
        y_pos += 4
    if linkedin:
        # Pulisce l'URL tenendo solo l'handle di LinkedIn e accorciandolo se troppo lungo
        in_handle = str(linkedin).replace("https://www.linkedin.com/in/", "").replace("https://linkedin.com/in/", "").replace("www.linkedin.com/in/", "").strip("/")
        if len(in_handle) > 28:
            in_handle = in_handle[:25] + "..."
        pdf.set_xy(6, y_pos)
        pdf.cell(48, 4, f"in: {in_handle}", 0, 1, 'L')
    
    # QR Code Spostato a destra
    pdf.image(qr_path, 56, 12, 24)

    # ==========================================
    # LATO 2: RETRO (Azzurro Istituzionale)
    # ==========================================
    pdf.add_page()
    pdf.set_fill_color(*BLUE_ASSAFRICA)
    pdf.rect(0, 0, 85, 55, 'F')
    
    # Logo molto più grande (Largo 45mm, centrato)
    logo_path = conf.get('logo_negativo', '')
    if logo_path and os.path.exists(logo_path):
        pdf.image(logo_path, 20, 10, 45)
    else:
        pdf.set_xy(0, 22)
        pdf.set_font("helvetica", "B", 12)
        pdf.set_text_color(255, 255, 255)
        pdf.cell(85, 6, nome_ass.upper(), 0, 1, 'C')

    # Contatti Istituzionali sul retro (Indirizzo e Mail generale)
    pdf.set_xy(0, 39)
    pdf.set_font("helvetica", "", 6)
    pdf.set_text_color(200, 200, 200)
    if indirizzo:
        pdf.cell(85, 3, indirizzo, 0, 1, 'C')
    if email_ass:
        pdf.cell(85, 3, email_ass, 0, 1, 'C')

    # Sito Web in grassetto
    pdf.set_xy(0, 46)
    pdf.set_font("helvetica", "B", 8)
    pdf.set_text_color(255, 255, 255)
    pdf.cell(85, 4, sito, 0, 1, 'C')

    # Salvataggio
    file_path = f"exports/Biglietto_{nome}_{cognome}.pdf"
