...
def genera_report_dati(df):
    ...
    for _, row in df.sort_values(by=['pagato', 'nome'], ascending=[False, True]).iterrows():
        ...
        # FIX UNICODE APPLICATO A TUTTI I CAMPI TESTUALI
        nome_p = str(row['nome']).encode('latin-1', 'ignore').decode('latin-1') if pd.notna(row['nome']) else ""
        cat_p = str(row['categoria']).encode('latin-1', 'ignore').decode('latin-1') if pd.notna(row['categoria']) else ""
        ref_val = str(row['referente']) if pd.notna(row['referente']) else ""
        ref_p = ref_val.encode('latin-1', 'ignore').decode('latin-1')
        
        pdf.cell(w_nome, 7, f" {nome_p[:40]}", 'B', 0, "L", fill)
        pdf.cell(w_cat, 7, f" {cat_p[:30]}", 'B', 0, "L", fill)
        pdf.cell(w_ref, 7, f" {ref_p[:25]}", 'B', 0, "L", fill)
